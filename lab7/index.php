...
<body>
    <div class="myForm">
        ...
        <form action="index.php" autocomplete="off" method="post">
            <div class="container">
                *<input type="text" placeholder="Numele" name="name" class="half" value="<?php if (!empty($_POST['name'])) {
                    echo $name;
                } ?>" />
                *<input type="text" placeholder="Prenumele" name="surname" class="half" value="<?php if (!empty($_POST['surname'])) {
                    echo $surname;
                } ?>" />
                *<input type="text" placeholder="Număr telefon" name="phone" class="half" value="<?php if (!empty($_POST['phone'])) {
                    echo $phone;
                } ?>" />
                *<input type="email" placeholder="Adresa electronică" name="email" class="half" value="<?php if (!empty($_POST['email'])) {
                    echo $email;
                } ?>" />
                <div class="group">
                    <label>Data venirii *<input type="date" name="int_date" value="<?php if (!empty($_POST['int_date'])) {
                        echo $int_date;
                    } ?>" /></label>
                </div>
                <div class="group">
                    <label>Data plecării *<input type="date" name="exit_date" value="<?php if (!empty($_POST['exit_date'])) {
                        echo $exit_name;
                    } ?>" /></label>
                </div>
                <div class="group">
                    <label>Număr persoane *<input type="number" value="1" min="1" max="5" name="nmbr" value="<?php if (!empty($_POST['nmbr'])) {
                        echo $nmbr;
                    } ?>" /></label>
                </div>
                <div class="group">
            ...
            <textarea placeholder="Observații și doleanțe" rows="5" name="obs"><?php if (!empty($_POST['obs'])) {
                echo $obs;
            } ?></textarea>
